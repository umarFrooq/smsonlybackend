...
describe('User routes', () => {
  describe('POST /v1/users', () => {
    ...
    test('should return 201 and successfully create new student with grade and section', async () => {
      await insertUsers([admin]);
      await insertGrades([gradeOne]);
      newUser.role = 'student';
      newUser.gradeId = gradeOne._id;
      newUser.section = gradeOne.sections[0];
      ...
      expect(res.body.gradeId).toBe(gradeOne._id.toHexString());
      expect(res.body.section).toBe(gradeOne.sections[0]);

      const dbUser = await User.findById(res.body.id);
      expect(dbUser.role).toBe('student');
      expect(dbUser.gradeId.toHexString()).toBe(gradeOne._id.toHexString());
      expect(dbUser.section).toBe(gradeOne.sections[0]);
    });
  });
  ...
  describe('GET /v1/users', () => {
    ...
    test('should correctly apply filter on fullname field', async () => {
      await insertUsers([userOne, userTwo, admin]);

      const res = await request(app)
        .get('/v1/users')
        .set('Authorization', `Bearer ${adminAccessToken}`)
        .query({ fullname: userOne.fullname })
        .send()
        .expect(httpStatus.OK);
  ...
  describe('PATCH /v1/users/:userId', () => {
    test('should return 200 and successfully update user if data is ok', async () => {
      ...
      expect(dbUser).toMatchObject({ fullname: updateBody.fullname, email: updateBody.email, role: 'user' });
    });

    test('should return 401 error if access token is missing', async () => {
      await insertUsers([userOne]);
      const updateBody = { fullname: faker.name.findName() };
